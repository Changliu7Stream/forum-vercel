/**
 * 图标替换脚本
 * 将页面中所有 <i class="icon-xxx"></i> 元素替换为对应的内联 SVG 图标
 * 替代 Lucide 字体图标，避免外部字体依赖
 */
(function() {
  // Lucide 图标 SVG 路径映射（简化版，使用 stroke 风格）
  const iconPaths = {
    ...
    'pin': '<path d="M12 17v5"></path><path d="M9 10.76a2 2 0 0 1-1.11 1.79l-1.78.9A2 2 0 0 0 5 15.24V17h14v-1.76a2 2 0 0 0-1.11-1.79l-1.78-.9A2 2 0 0 1 15 10.76V6h1a2 2 0 0 0 0-4H8a2 2 0 0 0 0 4h1z"></path>',
  ...
  function replaceAllIcons() {
    ...
    // 转为数组，因为 replaceChild 会修改 DOM
    const elements = Array.from(iconElements);
    elements.forEach(replaceIcon);
  }
  ...
  // 暴露给动态渲染使用
  window.replaceIcons = replaceAllIcons;

  // 提供给 animated-icons.js 的本地 SVG 生成接口（替换 CDN 依赖）
  window.IconSVG = {
    getPath: function(name) { return iconPaths[name] || null; },
